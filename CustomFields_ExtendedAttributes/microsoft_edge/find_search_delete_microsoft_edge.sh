...
# SCRIPT
#################################################################################
# Find and delete all instances of Microsoft Edge.app

if_app_is_below_this_version_then_delete_it="114.0.1823.41"

mdfind kind:application "Microsoft Edge.app" | while read -r  file; do
printed-line
	if [[ "$file" == "/Applications/Microsoft Edge.app" ]]; then
			writelog "we found /Applications/Microsoft Edge.app"
			version_installed_app=$(defaults read "$file/Contents/Info.plist" CFBundleShortVersionString)
			writelog "Microsoft Edge version: $version_installed_app"
